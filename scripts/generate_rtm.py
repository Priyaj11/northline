...
#: The same matrix with a result column, written after a run. NOT committed,
#: because its content depends on which reports happen to be on disk.
#:
#: The matrix serves two purposes that were quietly merged until continuous
#: integration caught it. Its structure is a VIEW of the registers, so it must
#: be deterministic and checkable for drift. Its results are a RECORD of one
#: run, so they vary legitimately. One file cannot be both: the drift check
#: failed because the committed copy held results from a local run and the
#: pipeline regenerated it with an empty reports directory.
RESULTS_OUTPUT = REPO_ROOT / "reports" / "traceability-with-results.md"
...
def main() -> int:
        ...
        for req in area_reqs:
            ...
            for case in matched:
                ...
                lines.append(
                    ...
                    f"{automation} | {defect_cell} | see the results copy |"
                )
                result_rows.append(
                    f"| {req['id']} | {case['id']} {case['title']} | "
    ...
    RESULTS_OUTPUT.write_text("\n".join([
        "# Traceability matrix with results",
        "",
        "Generated by scripts/generate_rtm.py after a run. NOT committed: results",
        "depend on which reports are on disk, so this file changes legitimately every",
        "time. The committed matrix in docs/ holds the register-derived columns only.",
        "",
        "Only test cases naming a specific automated test carry a real outcome. The",
        "rest read 'no explicit link'.",
        "",
        "| Requirement | Test case | Automation | Defect | Result |",
        "| --- | --- | --- | --- | --- |",
    ] + result_rows + [""]))
