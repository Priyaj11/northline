...
## The predicted risks did not materialise

The known risks section above was written before the pipeline had ever run. On
the first execution, on an ubuntu-latest runner, the test job passed in 2
minutes 12 seconds and none of the three predicted problems occurred:

    ParaBank start time      started and seeded within the job timeout
...
they were predictions and the evidence says otherwise. Leaving a list of
predicted problems in a document after the evidence contradicts it is how
documentation becomes folklore.

The one thing that DID fail was not on the list: a generated document carrying a
timestamp, which made the drift check impossible to satisfy. Recorded above.
